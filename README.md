...
Windows 在默认音频端点变化或格式重协商时会**静默地停掉采集流**——开会时很常见。
原本的表现是：程序看起来一切正常，但从此再也不出字幕，日志里没有任何记录。

现在 VAD 线程顺带监控：非暂停状态下超过 6 秒没有音频回调，就记录错误、
在字幕窗上提示「音频中断，正在重连」，并自动重开设备。

阈值 6 秒是安全的——实测 WASAPI 回录在**完全静音时仍以 50 次/秒**产生回调
（填充静音数据），所以没有声音不会误触发。这一点是先验证过才敢加的：
一个会每 6 秒重开一次设备的看门狗，比它要修的 bug 更糟。
...
这条是从一次真实误诊里来的：托盘程序已经在跑的时候再双击，屏幕上什么都不会变，
...
托盘菜单里还有「**查看日志**」，直接打开 `logs/app.log`。
启动失败时它是唯一的线索：有新时间戳说明程序起来了只是没看见，
没有新内容说明死在 import 阶段——两种完全不同的故障。
...
app/
  ...
  single_instance.py    单实例守卫（QLocalServer 命名管道）
  cli.py                终端前端
  ui/                   Qt 悬浮窗前端
