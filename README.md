...
### 整机功耗与续航（拔电，电量差分，5 分钟窗口，满电 73.4 Wh）

这组数字测于优化前，尚未复测：

| 阶段 | 整机功耗 | 预计续航 |
|---|---|---|
| 基线（应用关闭） | 5.40 W ±1.32 | ~13.6 h |
| 应用满负荷翻译 | 14.75 W ±1.32 | ~5.0 h |

按 SoC 满负荷下降 6.3 W 推算，优化后整机应在 9~10 W 左右、续航 7~8 小时，
但**这是推算不是实测**。要确认请拔电后跑：

```bash
python tools/measure_power.py --battery --skip-idle --seconds 300
```
...
tests/                  67 个单测，不需要声卡（VAD/配置测试需要本机模型文件）
...
```bash
python -m pytest tests/ -q                    # 67 个单测，秒级
