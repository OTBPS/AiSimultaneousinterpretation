...
笔记本上这是硬指标，所以和延迟一样实测，不估算。`tools/measure_power.py` 跑三段对比。

Intel RAPL（**只含 SoC，不含屏幕、WiFi、风扇**）：

| 阶段 | SoC 总计 | CPU | 核显 |
|---|---|---|---|
| 基线（应用关闭） | 11.97 W | 6.39 W | 0.01 W |
| 应用待机（监听中，无人说话） | 10.85 W | 5.15 W | **0.01 W** |
| 应用满负荷翻译 | 19.39 W | 8.30 W | **4.75 W** |

**待机几乎不耗电。** 核显读数 0.01 W —— 没人说话时 VAD 之外什么都不跑，而 VAD 在 CPU 上
只要 102 µs/帧。待机那一档比基线还低 1.1 W，这是测量噪声（基线阶段机器上有别的活动），
说明真实待机开销小到淹没在噪声里。

**满负荷翻译约 +7.5 W**（相对基线），其中核显 4.75 W、CPU 增加约 1.9 W。
注意这是**连续不停说话**的极端情况；真实会议有停顿，平均功耗低于此。

整机功耗（拔掉充电器，电量差分法，5 分钟窗口，满电容量 73.4 Wh）：
...
**连续不停翻译大约把续航砍到三分之一**（13.6 h → 5.0 h）。但这是极端情况：
真实会议有停顿，静音时开销回落到接近 0。按 60% 语音占比估算平均约 11 W，续航约 6.7 h。

注意整机增量（9.36 W）大于 SoC 增量（7.02 W），差的 2.3 W 是 RAPL 看不到的部分：
风扇转起来、内存控制器、供电损耗，以及测试时的音频播放本身。

### 测量方法上的坑

不要用 `DischargeRate` 求平均。这台机器的电量计**约 30 秒才刷新一次**，60 秒窗口里只有
2 个不同读数，对阶梯信号取平均会得出「翻译时反而省电」这种荒谬结论（实际测到过）。

改用**电量差分**：窗口内消耗的 mWh ÷ 时间。这是累积量，不受刷新延迟影响。代价是量化步长
约 110 mWh，所以窗口必须够长：

| 窗口 | 量化误差 |
|---|---|
| 60 s | ±1.32 W ← 和被测差值同量级，无意义 |
| 300 s | ±1.32 W → 相对 9.36 W 的差值约 28%，可用 |
| 600 s | ±0.66 W |

工具会自动计算并打印这个误差，当误差相对被测差值过大时直接警告。
...
tools/
  ...
  measure_power.py      功耗三段对比（RAPL + 电池放电）
